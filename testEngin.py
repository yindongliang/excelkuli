...
def makeImgFolder(custom_foldername):
    ...
    if custom_foldername =='':
        imgPath = ".\\screenshot\\" + currentTime
    else:
        imgPath = ".\\screenshot\\" +custom_foldername+"\\" +currentTime
...
def extendWaitEvent(image,offsetDirect="left",offsetValue=0,event="click",inputContent="",timeOut=10,hotKey="",timeOutContinue=""):
    ...
    if findFlg != 1:
        ...
        if not hotKey == "":
            ...
            elif event=="loopfindClick":
                loopfindClick(image,"click",inputContent)
                hover(screenX)
            elif event=="type":
                type(inputContent)
            else:
...
def loopfindClick(image, eventType="click", inputContent="0"):
    print inputContent
    if 1:
        indx=1
